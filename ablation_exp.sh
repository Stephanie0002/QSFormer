...
for method in "${methods_list[@]}"; do
    # 去除自适应小批次生成
    cmd="python ../FastDygForm/train_link_prediction.py --dataset_name $data --gpu $gpu --order chorno --model_name $method --num_runs 1 --num_hops 2 --num_layers 2 --load_best_configs --ablation --test_interval_epochs 120"
    echo "去除自适应小批次生成"
    echo "$cmd"
    eval "$cmd"
    ...
    # 去除身份编码
    cmd="python ../FastDygForm/train_link_prediction.py --dataset_name $data --gpu $gpu --order gradient-0.08-3 --model_name $method --num_runs 1 --num_hops 2 --num_layers 2 --no_id_encode --load_best_configs --ablation --test_interval_epochs 120"
    echo "去除身份编码"
    echo "$cmd"
    eval "$cmd"
