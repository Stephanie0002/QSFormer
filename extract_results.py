...
for root, dirs, files in os.walk(folder_path):
    ...
    # if files:
        ...
        with open(file_path, 'r') as file:
            ...
            # 使用正则表达式提取时间值
            matches = re.finditer(r"(train time|val time|load feature time|encodeCo time|construct patchs time|forward time|neighbor sample time|backward time):(\d+.\d+)", content)
