...
class FastFormer(nn.Module):
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray, no_time: bool=False):
        ...
        src_padded_nodes_neighbor_co_occurrence_features = self.frequency_encoder(src_padded_nodes_appearances).sum(dim=2)
            # self.frequency_encoder(src_padded_nodes_appearances).sum(dim=2)
        ...
        dst_padded_nodes_neighbor_co_occurrence_features = self.frequency_encoder(src_padded_nodes_appearances).sum(dim=2)
            # self.frequency_encoder(dst_padded_nodes_appearances).sum(dim=2)
