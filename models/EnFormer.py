...
class EnFormer(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
        ...
        if not no_id_encode:
            self.projection_layer = nn.ModuleDict({
                ...
                'neighbor_co_occurrence': FeedForward(self.patch_size * (self.neighbor_co_occurrence_feat_dim+self.max_input_sequence_length), out_dims=self.channel_embedding_dim, dropout=0., use_single_layer=True)
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray, no_time: bool=False):
        ...
        src_nodes_neighbor_ids_list, src_nodes_edge_ids_list, src_nodes_neighbor_times_list = \
            self.neighbor_sampler.get_historical_neighbors(node_ids=src_node_ids, node_interact_times=node_interact_times, num_neighbors=self.max_input_sequence_length-1)
        ...
        src_padded_nodes_appearances, dst_padded_nodes_appearances = count_nodes_cooccurrence(src_padded_nodes_neighbor_ids, dst_padded_nodes_neighbor_ids)
        # src_padded_nodes_appearances, Tensor, shape (batch_size, src_max_seq_length, 2)
        src_padded_nodes_appearances = torch.from_numpy(src_padded_nodes_appearances).float().to(self.device)
        ...
        if not self.no_id_encode:
            ...
            src_neigh_mask = np.expand_dims(src_padded_nodes_neighbor_ids, axis=1) == np.expand_dims(src_padded_nodes_neighbor_ids, axis=2)
            src_neigh_mask = torch.from_numpy(src_neigh_mask)
            src_iden_encode = src_neigh_mask.float().to(self.device)
            ...
            dst_neigh_mask = np.expand_dims(dst_padded_nodes_neighbor_ids, axis=1) == np.expand_dims(dst_padded_nodes_neighbor_ids, axis=2)
            dst_neigh_mask = torch.from_numpy(dst_neigh_mask)
            dst_iden_encode = dst_neigh_mask.float().to(self.device)
...
class NeighborCooccurrenceEncoder(nn.Module):
    ...
    def forward(self, src_padded_nodes_neighbor_ids: np.ndarray, dst_padded_nodes_neighbor_ids: np.ndarray):
        ...
        Co_Counter_CPP_AVAILABLE = True
        try:
            from utils.cpp.src.cpp_cores import count_nodes_cooccurrence
        except ImportError as e:
            Co_Counter_CPP_AVAILABLE = False
            print(str(e))
        if Co_Counter_CPP_AVAILABLE:
            src_padded_nodes_appearances, dst_padded_nodes_appearances = count_nodes_cooccurrence(src_padded_nodes_neighbor_ids, dst_padded_nodes_neighbor_ids)
            src_padded_nodes_appearances = torch.from_numpy(src_padded_nodes_appearances).float().to(self.device)
            dst_padded_nodes_appearances = torch.from_numpy(dst_padded_nodes_appearances).float().to(self.device)
        else:
