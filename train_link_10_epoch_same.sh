...
for method in "${methods_list[@]}"; do
    for data in "${data_list[@]}"; do
        ...
        if ! find $dir_to_check -type f -name "*.same.efficency" -print -quit | grep -q '.'; then
            ...
            cmd="CUDA_VISIBLE_DEVICES=$gpu python train_link_prediction.py --num_runs 1 --dataset_name $data --model_name $method  --num_epochs 10 --max_input_sequence_length 256 --patch_size 8 --num_hops 2 --val_neg_size 1 --test_neg_size 1 --test_interval_epochs 20"
            echo "$cmd"
            eval "$cmd"
            if [ $? -ne 0 ]; then
